// Would You Rather Setup page - Second step in the flow
...
export default function WouldYouRatherSetupScreen() {
  ...
  useEffect(() => {
    animateFade();
    setSelectedGameId('would_you_rather');
  }, []);
  ...
  const handleAddCustomQuestion = () => {
    if (!customQuestion.trim() || !customOptionA.trim() || !customOptionB.trim()) {
      Alert.alert('Error', 'Please enter a question and both options');
      return;
    }

    if (!customPackId) {
      Alert.alert('Error', 'Please select a pack for this question');
      return;
    }

    // Store custom question in session or local storage
    ...
    Alert.alert('Success', `Custom question added to ${PACKS.find(p => p.id === customPackId)?.name} pack`);
    setCustomQuestion('');
    setCustomOptionA('');
    setCustomOptionB('');
  ...
  const handleNext = () => {
    ...
    // Navigate to game
    router.push('/would-you-rather' as any);
  };
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalLabel}>Question (optional):</Text>
            <TextInput
            ...
            <Text style={styles.modalLabel}>Option A:</Text>
            <TextInput
              style={styles.modalInput}
              placeholder="Enter first option"
              placeholderTextColor={theme.colors.textMuted}
              value={customOptionA}
              onChangeText={setCustomOptionA}
              multiline
              numberOfLines={2}
            />

            <Text style={styles.modalLabel}>Option B:</Text>
            <TextInput
              style={styles.modalInput}
              placeholder="Enter second option"
              placeholderTextColor={theme.colors.textMuted}
              value={customOptionB}
              onChangeText={setCustomOptionB}
              multiline
              numberOfLines={2}
            />
